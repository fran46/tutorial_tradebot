...
if (empty($_SESSION['steam_uptodate']) or empty($_SESSION['steam_personaname'])) {
	...
	if(isset($content['response']['players'][0]['steamid'])) {
		...
		if(mysqli_num_rows($check_user)==1) {
			...
			$_SESSION["baneado"]=$row_check_user[4];
		} else {
			...
			(".$_SESSION['steam_steamid'].",'".$_SESSION['steam_personaname']."','".$_SESSION['steam_avatar']."','".$_SESSION['steam_avatarmedium']."','".$fecha_registro."','".$_SERVER['REMOTE_ADDR']."','".$_SERVER['REMOTE_ADDR']."','".$_hash."')");
			$_SESSION["acepta_tos"]=0;
